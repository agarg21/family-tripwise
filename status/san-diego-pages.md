...
The review-clean promoted authority pilot is `FT-AUTH-005` / `IMP-042`, pending explicit user approval. It would add a compact, source-dated 12-attraction logistics index and client-side download generated from embedded page data to the existing all-ages page. It would not create a separate data URL or another canonical/indexable HTML page. Outreach remains a separate later user gate.
...
|---|---|---:|---|---|---|---|---|---|---|
| Main things-to-do hub | https://familytripwise.com/things-to-do/san-diego-with-kids.html | 4.4K+ known core demand | No page row; URL Inspection says unknown to Google on July 31 | Top 20 first, then Top 10 | `things to do in San Diego with kids`; `best things to do in San Diego with kids`; `San Diego with kids activities` | Live; the 12-activity `FT-IMP-005` revision is released and production-verified at `31bba30` through Pages run `29876597948`. `FT-AUTH-004` selects it as the best existing-page citation asset. | Keeps the persona router and compares 12 primary activities. Six new major/day-trip choices receive compact logistics notes without repeating every existing attraction in another long layer. | Discovery/indexing remains the first search blocker. The proposed authority enhancement and every outreach contact remain user-gated. | `FT-AUTH-005` / `IMP-042`: after explicit approval, add one source-dated 12-attraction logistics index and client-side data download generated from embedded page data; no separate data URL or new HTML page. |
...
`FT-IMP-005` is released and production-verified at `31bba30` through Pages run `29876597948`. `FT-EVAL-001` remains superseded. `FT-AUTH-004` promotes `FT-AUTH-005` / `IMP-042` as the next bounded San Diego growth option, but it is not selected and requires explicit user approval. The lodging measurement item remains non-blocking `FT-EVAL-002`: both revised URLs now have current crawl evidence, but run it only after protected page/query evidence includes the July 31 stay-area crawl, and use it to assess area-vs-hotel query interpretation rather than claim short-window ranking causality.
