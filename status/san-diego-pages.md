...
`FT-IMP-005` is released and production-verified at `31bba30` through Pages run `29876597948`. `FT-EVAL-001` remains superseded. `FT-AUTH-005` / `IMP-042` is released and production-verified at `e7c40ff` through Pages run `30667262994`. The lodging measurement item remains non-blocking `FT-EVAL-002`: both revised URLs now have current crawl evidence, but run it only after protected page/query evidence includes the July 31 stay-area crawl, and use it to assess area-vs-hotel query interpretation rather than claim short-window ranking causality.
