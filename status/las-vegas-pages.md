...
- Current public-safe GSC snapshot: `ops/gsc-snapshots/2026-08-27.md`
...
Demand estimates are directional US monthly search volumes from the logged-in Semrush check collected July 22, 2026. Closely related rows overlap and must not be summed into unique traffic. The GSC evidence column uses the August 27 snapshot finalized through August 25. Page-average position is not a claim that a page ranks at that position for every example query. Example queries are research targets, not a protected GSC query export.
...
Las Vegas remains complete at the current five-page core-cluster level. `FT-RES-015` confirms that the broad all-ages activity variants belong on one existing URL without declaring the July revision a ranking failure. `FT-AUTH-006` implements the bounded usefulness/citation improvement on that URL. `FT-RES-021` now finds that four current family questions need a joined base-and-day-shape decision, not more inventory; the existing pages own its components. Preserve all five page roles. The cross-page pool-versus-central-base task remains an unselected research hypothesis, and no observation or edit should be manufactured from cadence.
