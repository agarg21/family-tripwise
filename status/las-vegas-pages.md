...
Las Vegas is the next city. `FT-IMP-006` / `IMP-020` is completed and review-clean: the existing all-ages hub has a researched 12-choice compact implementation while its URL and role remain unchanged. The family-hotel page is the next separate research/build action; it was not part of `FT-IMP-006` and is not started by this status update.
