...
Demand estimates are directional US monthly search volumes from the logged-in Semrush check collected July 22, 2026. Closely related rows overlap and must not be summed into unique traffic. The GSC evidence column uses the August 28 snapshot finalized through August 26. Page-average position is not a claim that a page ranks at that position for every example query. Example queries are research targets, not a protected GSC query export.
...
The four current page roles already own the component decisions. The remaining unselected hypothesis is cross-page task completion: can one short pool-versus-central-base case and one four-day fixed-anchor case move from itinerary and activities through area and named-hotel checks without an avoidable detour or contradiction? Preserve all pages now; any implementation needs a separately selected task walkthrough.
