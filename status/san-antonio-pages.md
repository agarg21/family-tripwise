...
|---|---|---|---|---|---|---|---|---|---|
| Main things-to-do hub | https://familytripwise.com/things-to-do/san-antonio-with-kids.html | Exact measured phrases: 5,400, 1,600, and 880/month; variants overlap | Avg position 64.36; 11 impressions | Top 20 first, then Top 10 | `things to do in san antonio with kids`; `things to do with kids in san antonio`; `san antonio things to do with kids` | Review-clean 13-choice implementation; canonical/indexable URL and one sitemap entry preserved; release pending. | `FT-RES-010` covers SERPs, competitors, personas, and every section. `SRR-028` reviews 23 candidates, retains 13, traces `SA-P1` through `SA-P5`, and records current direct sources and claim boundaries. Pasteur cycle two returned `PASS` with no P0-P3. | Focused tests pass 5/5; full tests pass 110/110; state, SEO, responsive/sticky-table, 16-source, JSON, privacy/scope, and whitespace QA pass. Exact-path release and production verification remain. | Finish `FT-IMP-018`; do not start the hotel page in this transaction. |
...
| Future family hotel comparison | Proposed: https://familytripwise.com/where-to-stay/san-antonio-family-hotels.html | Exact measured phrases: 170, 140, and 110/month; resort/lazy-river variants add distinct wording but overlap | N/A: URL does not exist | Top 20 first, then Top 10 | `family friendly hotels in san antonio`; `best family hotels in san antonio`; `san antonio resorts for families` | Approved page role only; not researched or published. | `FT-RES-010` retains three exact shared URLs plus one domain-only recurrence between the two core hotel samples and a shared named-property job for resorts. `SA-P2` through `SA-P4` define family fields. | Requires a durable candidate/evidence pack with official room facts, broad total-night ranges, review observations/conflicts, freshness, maps, and unresolved checks. New page publication still needs normal authority. | Research after the all-ages improvement; use one hotel page with resort/lazy-river/pool/room filters, not variant URLs. |
...
1. Active: release and production-verify the review-clean `FT-IMP-018` all-ages hub.
2. Then: research and, only if review-clean and separately authorized, publish one family-hotel comparison.
...
No San Antonio page is protected by an active observation window. Early GSC evidence is too sparse for causal evaluation, so useful separately authorized improvements need not wait for an artificial hold. `FT-IMP-018` remains the only active San Antonio transaction.
