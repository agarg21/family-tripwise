...
| Stay areas / base | https://familytripwise.com/where-to-stay/new-york-city-with-kids.html | 30 exact kids variant; 70 `with a family` variant | Avg position 49.73; 49 impressions | Top 20 | `where to stay in New York City with kids`; `where to stay in NYC with a family`; `best area to stay in NYC for families` | Indexed; `FT-IMP-016` is review-clean locally on the existing URL while keeping it separate from hotels. | `SRR-025` refreshed current SERPs, ranking-page usefulness, all five personas, qualitative parent constraints, current primary sources, every visible section, and the area/hotel boundary. Maxwell returned `PASS` in cycle 2 with no P0-P3 findings. | Exact-path release and production verification remain. Full tests pass 101/101; operator-state and local SEO QA pass; 1280/390/320 responsive checks pass. | Release and production-verify `FT-IMP-016`; do not start the itinerary in the same transaction. |
...
| Where to stay / best area | Existing area/base page; active `FT-IMP-016` improvement keeps the same URL. |
...
4. Active: simplify the existing stay-area guide and route named hotels.
5. Next: simplify the existing itinerary into concrete day plans.
