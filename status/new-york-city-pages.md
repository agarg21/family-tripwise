...
| Teen/tween activities | https://familytripwise.com/things-to-do/new-york-city-with-teens.html | Exact measured phrases: 590, 480, and 170/month; variants overlap | N/A: proposed URL | Top 20 first, then Top 10 | `things to do in NYC with teens`; `things to do with teens in NYC`; `best things to do with teens in NYC` | `FT-IMP-015` completed and review-clean; exact-path release pending. | `SRR-024` reviewed the distinct SERP, representative competitors/community, NYC-P4, 20 candidate roles, current primary sources, claim boundaries, mixed-age support, every planned section, responsive behavior, and acceptance/measurement. Curie returned `PASS_WITH_P3` in cycle 2 with no P0-P2 findings. | Exact-path release and production verification remain. One non-blocking P3 notes the generic but licensed/optimized Central Park hero. | Release and verify `FT-IMP-015`; do not start the stay-area page in the same transaction. |
...
| Teens/tweens | One standalone specialist review-clean; release pending. |
