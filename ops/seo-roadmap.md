...
- No September 11 public-safe GSC snapshot is available at the 05:01 Eastern selection point. Recent reused September 10 authenticated read-only API evidence is finalized through September 8: 1,786 property impressions, 3 clicks, aggregate average position 63.36, and 24/28 inspected URLs indexed. The public snapshot has no aligned query cohort, so it remains crawl/orientation evidence only. The verified production layout defect, not GSC performance, selects the action. September 11's eligible learning unit remains due for the afternoon run.
