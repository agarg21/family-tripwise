...
- Latest snapshot: `ops/gsc-snapshots/2026-09-03.json`.
- Classification on September 3: fresh authenticated read-only Search Console API evidence, collected September 3 with finalized performance data through September 1. It arrived mechanically before `FT-RES-029` selection and passed public-schema/privacy validation.
...
- The sitemap was successfully resubmitted on July 27 and still reports `Success` with 28 discovered pages. Read-only URL Inspection in the September 3 snapshot found 24 indexed and four not indexed. The remaining not-indexed URLs are the San Diego all-ages and teen activity pages, New York City itinerary, and Chicago teen page.
- The finalized 28-day property total is 1,991 impressions, 3 clicks, 0.15% CTR, and aggregate average position 65.8. The public snapshot still has no aligned query cohort. These rolling page/property averages do not identify a CTR problem, ranking failure, or user-facing deficiency.
