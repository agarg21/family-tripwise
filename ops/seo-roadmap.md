...
- Latest snapshot: `ops/gsc-snapshots/2026-09-05.json`.
- Classification on September 6: recent reused authenticated read-only Search Console API evidence, collected September 5 with finalized performance data through September 3. No September 6 snapshot was available at selection time; the September 5 record passed public-schema/privacy validation.
...
- The sitemap was successfully resubmitted on July 27 and still reports `Success` with 28 discovered pages. Read-only URL Inspection in the September 5 snapshot found 24 indexed and four not indexed. The remaining not-indexed URLs are the San Diego all-ages and teen activity pages, New York City itinerary, and Chicago teen page.
- The finalized 28-day property total is 1,829 impressions, 1 click, 0.05% CTR, and aggregate average position 65.21. The public snapshot still has no aligned query cohort. These rolling page/property averages do not identify a CTR problem, ranking failure, or user-facing deficiency.
- URL Inspection records August 31 crawls for both indexed Chicago target pages. The all-ages page has 20 impressions at page-average position 57.65 and the itinerary has 37 at 54.11; ranking or CTR interpretation still requires sufficient finalized aligned query evidence.
