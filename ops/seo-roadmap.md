...
- September 11 morning action: `FT-MAINT-005` / `IMP-053` is locally implemented and review-clean pending release. The verified shared San Diego hero defect affects all five generated pages, not only the two `FT-RES-039` task targets. The owning city record and renderer now emit the image's verified 3,766 by 2,509 dimensions, while one target-specific auto-height rule preserves the prior responsive geometry. The first dimensions-only attempt failed controlled mobile QA with a 520-pixel box and was corrected before review or release. Delayed-image checks on all five targets show zero hero-height or following-element movement at 1280 by 900, 390 by 844, and 320 by 800, stable 1,160-by-520 / 354-by-about-236 / 284-by-about-189 rendered boxes, the existing crop, and no overflow. Focused `23/23`, full `164/164`, deterministic-generation, state, freshness, local/production SEO, 59-snapshot, public-preflight, strict-JSON, exact-scope, browser, privacy, and whitespace checks pass. Lagrange cycle one found one P2 stale future-action sentence in `ops/needs-user.md`; cycle two verified its removal and returned `PASS` with no P0-P3. Every content, search, role, evidence, and measurement invariant remains fixed.
