...
- Latest GSC evidence: `ops/gsc-snapshots/2026-08-19.md`, fresh authenticated read-only API evidence with finalized data through 2026-08-17. The property has 1,905 impressions, 5 clicks, aggregate average position 64.18, 28 discovered pages, and 24 of 28 inspected URLs indexed. The rolling-window loss of one historical San Diego stay-area click, unchanged indexing, page-average positions and absence of a public query cohort do not create a CTR or rewrite action.
...
- GSC: authenticated read-only API collection is working; the August 19 snapshot has finalized data through August 17, 1,905 impressions and 5 clicks over 28 days, a successful sitemap read with 28 discovered pages, and 24 of 28 monitored URLs indexed
...
- Current authenticated API evidence is in `ops/gsc-snapshots/2026-08-19.json`, collected through the read-only API with finalized data through August 17; it supersedes earlier UI and API snapshots for current status.
...
- Latest public preflight on August 19: sitemap returned 200 with 28 parsed URLs; robots returned 200 and advertises the sitemap; all 28 configured URLs returned 200 and appeared in the sitemap.
- Current normalized source: `ops/gsc-snapshots/2026-08-19.json`, collected through the authenticated read-only API on August 19 with finalized data through August 17.
