...
- Latest GSC evidence: `ops/gsc-snapshots/2026-08-18.md`, fresh authenticated read-only API evidence with finalized data through 2026-08-16. The property has 1,935 impressions, 6 clicks, aggregate average position 64.04, 28 discovered pages, and 24 of 28 inspected URLs indexed. The rolling change, unchanged clicks/indexing, page-average positions and absence of a public query cohort do not create a CTR or rewrite action.
...
- GSC: authenticated read-only API collection is working; the August 18 snapshot has finalized data through August 16, 1,935 impressions and 6 clicks over 28 days, a successful sitemap read with 28 discovered pages, and 24 of 28 monitored URLs indexed
...
- Current authenticated API evidence is in `ops/gsc-snapshots/2026-08-18.json`, collected through the read-only API with finalized data through August 16; it supersedes earlier UI and API snapshots for current status.
...
- Latest public preflight on August 18: sitemap returned 200 with 28 parsed URLs; robots returned 200 and advertises the sitemap; all 28 configured URLs returned 200 and appeared in the sitemap.
- Current normalized source: `ops/gsc-snapshots/2026-08-18.json`, collected through the authenticated read-only API on August 18 with finalized data through August 16.
