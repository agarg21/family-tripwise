...
- Latest GSC evidence: `ops/gsc-snapshots/2026-08-20.md`, fresh authenticated read-only API evidence with finalized data through 2026-08-18. The property has 2,144 impressions, 5 clicks, aggregate average position 65.45, 28 discovered pages, and 24 of 28 inspected URLs indexed. Inspection/crawl state is unchanged; the larger rolling page/property impression totals and absence of a public query cohort do not create a CTR or rewrite action.
...
- GSC: authenticated read-only API collection is working; the August 20 snapshot has finalized data through August 18, 2,144 impressions and 5 clicks over 28 days, a successful sitemap read with 28 discovered pages, and 24 of 28 monitored URLs indexed
...
- Current authenticated API evidence is in `ops/gsc-snapshots/2026-08-20.json`, collected through the read-only API with finalized data through August 18; it supersedes earlier UI and API snapshots for current status.
...
- Latest public preflight on August 20: sitemap returned 200 with 28 parsed URLs; robots returned 200 and advertises the sitemap; all 28 configured URLs returned 200 and appeared in the sitemap.
- Current normalized source: `ops/gsc-snapshots/2026-08-20.json`, collected through the authenticated read-only API on August 20 with finalized data through August 18.
