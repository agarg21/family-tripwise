...
No user decision is required for completed, review-clean `FT-RES-028` / `SRR-049`. The research answers the recent demand-versus-impressions question using the fresh public snapshot and a fresh protected export aggregated only outside the repository. The result preserves all pages: current head-query exposure is far below researched monthly demand, role-aligned impressions average about position 72.8, four meaningful-demand pages remain not indexed, and property/page/query aggregates must not be treated as additive. Hilbert cycle-two `PASS` leaves no P0-P3 after three cycle-one corrections. No page, URL, title, indexing request, external action, destination, or automation change is selected; docs-only push is pending.
