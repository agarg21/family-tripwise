...
const target = "family-itinerary/new-york-city-with-kids.html";
...
test("keeps one canonical, indexable New York City family-itinerary page", () => {
  const html = readFileSync(join(root, "site", target), "utf8");
  const sitemap = readFileSync(join(root, "site", "sitemap.xml"), "utf8");

  assert.match(html, /<title>New York City Itinerary With Kids: Realistic 1, 2 and 3 Day Plans<\/title>/);
  assert.match(html, /<h1>New York City Itinerary With Kids: Realistic 1, 2 and 3 Day Plans<\/h1>/);
  assert.match(html, /<link rel="canonical" href="https:\/\/familytripwise\.com\/family-itinerary\/new-york-city-with-kids\.html">/);
  assert.doesNotMatch(html, /<meta[^>]+name="robots"[^>]+noindex/i);
  assert.equal((sitemap.match(/https:\/\/familytripwise\.com\/family-itinerary\/new-york-city-with-kids\.html/g) || []).length, 1);
  assert.match(html, /Attraction and route sources checked:<\/strong> July 26, 2026/);
});
...
test("aligns schema, visible FAQ, sources, image, and all four cluster routes", () => {
  ...
  assert.equal(itemList.name, "New York City family itinerary lengths");
  ...
  for (const required of [
    ...
    "../where-to-stay/new-york-city-family-hotels.html",
    "mta.info/accessibility/stations",
    "mta.info/elevator-escalator-status",
    "amnh.org/plan-your-visit/accessibility/sensory-resources-for-visitors",
    "1280px-Central_Park_New_York_City_New_York_23_cropped.jpg",
    "Jet Lowe",
...
test("regeneration is idempotent and changes no unrelated site file", () => {
  const temp = mkdtempSync(join(tmpdir(), "family-tripwise-nyc-itinerary-"));
