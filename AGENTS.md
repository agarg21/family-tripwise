...
- Daily snapshot ingestion, schema/privacy validation, routine URL Inspection reconciliation, and a healthy unchanged public preflight are run-start housekeeping. They may coexist with one substantive roadmap action and do not consume its allowance.
- Monitoring preempts substantive roadmap work only when it verifies a technical or production defect, identifies a safety or trust blocker, supplies a due substantive experiment decision, or is explicitly marked `preempts_substantive_action`. Otherwise record the monitoring result and continue to the highest-value eligible substantive item.
