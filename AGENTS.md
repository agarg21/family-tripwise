...
- From 2026-09-13 through 2026-11-13 at 17:00 America/New_York, the user authorizes this permanent Family Tripwise Master / Operator thread to run twice daily as the project's only scheduler through `family-tripwise-two-month-autopilot`. This renewal supersedes the completed 2026-08-12 through 2026-09-12 pilot. Do not consult, dispatch through, validate leases from, or write to the central SEO Control Room during this cycle.
...
- A scheduled run may select at most one substantive action. Across each America/New_York calendar day, the two runs must complete at least one bounded learning unit unless the cycle has ended or a specific recorded technical/external blocker makes evidence collection impossible. A learning unit is a source-dated result from current parent questions, a task-based existing-page walkthrough, live SERP/ranking-page analysis, evidence freshness/conflict research, or a useful community-answer draft. Unchanged health checks and narrative restatement do not count.
- Use a finish-first loop. One substantive action is one family problem taken through evidence, implementation, focused/full QA, independent review, commit and verified release; these stages are not separate action allowances. Its source-dated evidence or task validation can satisfy daily learning. Complete eligible unfinished work before starting another exploratory report. A supported `PRESERVE` decision still counts; never manufacture edits or a shipping quota.
- First actual run of the Eastern day: select the highest-priority eligible problem and learn what is needed to resolve it, implementing in the same transaction when the evidence is sufficient. Second actual run: deliver the highest-priority evidence-qualified improvement or finish its review/release. Further research instead of delivery must identify the exact unresolved decision and why no unblocked qualified candidate can proceed. Use actual day/run state, not assumed trigger hours, to assign these roles.
- A blocked item is not a project-wide stop. Record its exact failed command/tool, affected gate, owner, unblock condition and next check once. Do not retry unchanged policy denials or bypass security, switch tools to evade restrictions, weaken QA, or deploy merely to test. Continue an independent eligible item when one exists. After two consecutive runs with no deliverable and the same blocker, give one concise escalation and choose work that does not depend on it; do not manufacture more reports about the same blocker.
- Keep one current decision in the roadmap item and a concise current-cycle summary. Link evidence/review history rather than restating it across every document. Research must end with implement now, a candidate with exact remaining gate and next eligible run, preserve until a named trigger, or reject. At every run report delivered and production-verified, learned only, blocked/not shipped, and next action distinctly. Once per seven days, summarize verified user-facing improvements, candidate age and unresolved blockers; commit counts, snapshots and paperwork are not delivery metrics.
- A monitoring-only no-op is allowed only after that Eastern day's learning unit is complete, after the cutoff, or under the recorded hard-blocker exception. Record completed units in `backlog/product-learning-ledger.md` with the family decision, hypothesis, evidence class, result, confidence, reusable lesson, and next falsification trigger.
...
- Community and forum research is qualitative evidence. The autonomous lane may save concise, source-linked answer drafts in the validated `backlog/community-answer-drafts.json` queue, with policy and review semantics in `backlog/community-answer-drafts.md`. It must never auto-post, vote, message, create accounts, evade moderation, impersonate a parent/local/traveler, claim firsthand experience, or insert a promotional link that does not uniquely help. A queue decision of `approved` accepts wording for a possible later action; it does not authorize posting.
...
- Do not create or modify another recurring automation. After the cycle cutoff, perform no project work until the user reauthorizes it.
